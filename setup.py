...
etiket = tk.Label(text='Knight Online Notification', bg=backgroundcolor,
                  fg="#FFFFFF", pady=10, font=('Helvetica 8 bold'))
etiket.pack()
...
# labels
Label(form, text="X1:", font=('Helvetica 8 bold'),
      bg=backgroundcolor, fg='#FFFFFF', ).place(x=20, y=40)
Label(form, text="Y1:", font=('Helvetica 8 bold'),
      bg=backgroundcolor, fg='#FFFFFF').place(x=70, y=40)
Label(form, text="X2:", font=('Helvetica 8 bold'),
      bg=backgroundcolor, fg='#FFFFFF').place(x=20, y=80)
Label(form, text="Y2:", font=('Helvetica 8 bold'),
      bg=backgroundcolor, fg='#FFFFFF').place(x=70, y=80)
...
image = img
panel = tk.Label(form, image=img, relief="groove",
                 width=190, height=30, bg=backgroundcolor)
...
def image_check():
    ...
    while checkStatus:
        image = pyautogui.screenshot(
            region=(X1, Y1, int(X2)-int(X1), int(Y2)-int(Y1)))
...
def Dragging(event):
    x, y = event.x - lastClickX + form.winfo_x(), event.y - \
        lastClickY + form.winfo_y()
...
Button(form, text="-", command=hide, width=2, height=1, bg=backgroundcolor, fg="#FFFFFF",
       font=('Helvetica 8 bold'), borderwidth=1, cursor="hand2", activebackground="#2497E9") .place(x=256, y=0)
myButton = Button(form, text="X", command=form.destroy, width=2, height=1, bg=backgroundcolor, fg="#FFFFFF", font=(
    'Helvetica 8 bold'), borderwidth=1, cursor="hand2", activebackground="#f00").place(x=278, y=0)
form.mainloop()
